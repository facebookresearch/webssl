...
# Process the PyTorch state_dict to have the right key names before loading into the model
# For the non-processed teacher model
def load_pretrained_teacher_dino_weights(model, pretrained_weights, checkpoint_key):
...
if __name__ == '__main__':
    model_name = 'webssl-dino2b-full2b-224' # Replace with your favored model, e.g. webssl-dino1b-full2b-224 or webssl-dino7b-full8b-518
    model_name_with_underscore = model_name.replace('-', '_')

    # HuggingFace model repo name
    hf_model_name = f'facebook/{model_name}'
    # Path to local PyTorch weights
    pt_model_path = f'/checkpoint/amaia/video/davidfan/shared_checkpoints/webssl/{model_name_with_underscore}/{model_name_with_underscore}.pth'
